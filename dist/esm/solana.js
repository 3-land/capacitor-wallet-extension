...
export function signSerializedTransaction(serializedTransaction, signerPublicKey, signerSecretKey) {
    const transactionBytes = [...decodeBase58(serializedTransaction)];
    const signerPublicKeyBytes = [...decodeBase58(signerPublicKey)];
    const signatureLength = 64;
    const { value: signatureCount, nextOffset: signaturesStart } = decodeCompactLength(transactionBytes, 0);
    const messageStart = signaturesStart + signatureCount * signatureLength;
    if (messageStart > transactionBytes.length) {
        throw new Error('Serialized transaction is shorter than its signature header.');
    }
    const messageBytes = transactionBytes.slice(messageStart);
    const signerIndex = findSignerIndex(messageBytes, signerPublicKeyBytes, signatureCount);
    ...
    if (signatureEnd > transactionBytes.length) {
        throw new Error('Serialized transaction does not have space for the signer signature.');
    }
...
function decodeCompactLength(bytes, offset) {
    ...
    while (true) {
        if (index >= bytes.length) {
            throw new Error('Unexpected end of transaction while decoding compact length.');
        }
...
function findSignerIndex(messageBytes, signerPublicKey, signatureCount) {
    ...
    const { value: accountKeysCount, nextOffset: accountKeysStart } = decodeCompactLength(messageBytes, accountKeysLengthOffset);
    if (numRequiredSignatures > accountKeysCount) {
        throw new Error('Serialized transaction signer count exceeds account key count.');
    }
    if (numRequiredSignatures > signatureCount) {
        throw new Error('Serialized transaction signature array is smaller than the required signer count.');
    }
    ...
    throw new Error('The connected wallet is not a required signer on one of the provided transactions.');
}
